-- Migration 018: extend the `subject` check constraint on every table that
-- carries one, so 'business-studies' is a valid value alongside the existing
-- subjects.
--
-- Follows migration 017's pattern (drop-and-re-add idempotent CHECK). The
...
--   • skill_attempts
--   • diagnostic_results
--   • student_reports        (written by BusinessStudiesSession on session completion)
