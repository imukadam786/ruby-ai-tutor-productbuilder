-- Migration 017: extend the `subject` check constraint on every table that
-- carries one, so 'history' is a valid value alongside the existing subjects.
--
-- Follows migration 016's pattern (drop-and-re-add idempotent CHECK). The
...
--   • skill_attempts
--   • diagnostic_results
--   • student_reports        (written by HistorySession on session completion)
