...
import { getTopic } from "@/lib/accounting-selector";
...
async function handler(req: NextRequest) {
  ...
  try {
    const submission: AccountingSubmitAnswerRequest = await req.json();
    ...
    const { correct: isCorrect, score } = scoreAccountingAnswer(
    ...
    const response: AccountingSubmitAnswerResponse = {
    ...
    console.error("[accounting/submit-answer] error:", error);
...
export const POST = withRubies("accounting", handler);
