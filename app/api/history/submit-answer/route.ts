...
import { getTopic } from "@/lib/history-selector";
...
async function handler(req: NextRequest) {
  ...
  try {
    const submission: HistorySubmitAnswerRequest = await req.json();
    ...
    const { correct: isCorrect, score } = scoreHistoryAnswer(
    ...
    const response: HistorySubmitAnswerResponse = {
    ...
    console.error("[history/submit-answer] error:", error);
...
export const POST = withRubies("history", handler);
