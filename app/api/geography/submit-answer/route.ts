...
import { getTopic } from "@/lib/geography-selector";
...
async function handler(req: NextRequest) {
  ...
  try {
    const submission: GeographySubmitAnswerRequest = await req.json();
    ...
    const { correct: isCorrect, score } = scoreGeographyAnswer(
    ...
    const response: GeographySubmitAnswerResponse = {
    ...
    console.error("[geography/submit-answer] error:", error);
...
export const POST = withRubies("geography", handler);
