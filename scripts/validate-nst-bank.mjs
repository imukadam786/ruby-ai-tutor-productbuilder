// Validates the Natural Sciences & Technology consolidated bank against the
// skill tree and the BankQuestion item schema. Mirrors
// scripts/validate-life-skills-bank.mjs.
//
//   node scripts/validate-nst-bank.mjs                  (validate all)
//   node scripts/validate-nst-bank.mjs all              (same)
//   node scripts/validate-nst-bank.mjs NST.L4.LL.T01    (one topic)
//   node scripts/validate-nst-bank.mjs NST.L4.LL.T01 NST.L4.LL.T02
//
...
const BANK_PATH = path.join(ROOT, "data/nst-question-bank.json");
const TREE_PATH = path.join(ROOT, "data/nst-skill-tree.json");
...
function validateItem(item, idx, errors, warns) {
  ...
  // image key per option, same order). Files live at public/nst/<key>.<ext>.
...
function validateTopic(topicId) {
  ...
  if (!treeNode) {
    errors.push(`top: topic id not found in nst-skill-tree.json`);
  } else {
