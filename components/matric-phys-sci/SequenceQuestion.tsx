...
// learner's chosen order. Mirror of components/geography/SequenceQuestion.tsx;
// the only difference is the item type. The parent remounts this per question
// (via a `key`), so the chosen order resets between questions.

import { useMemo, useState } from "react";
import type { MatricPhysSciSequenceItem } from "@/types/matric-phys-sci";

interface Props {
  items: MatricPhysSciSequenceItem[];
  submitting: boolean;
...
export default function SequenceQuestion({ items, submitting, onSubmit }: Props) {
  ...
  const byId = useMemo(() => {
    const m: Record<string, MatricPhysSciSequenceItem> = {};
        ...
        <div className="flex flex-col gap-2 min-h-[44px]">
          ...
          {remaining.map((it) => (
            <button
              ...
              disabled={submitting}
              onClick={() => place(it.id)}
              className="min-h-[44px] px-4 py-3 rounded-2xl border-2 border-sky-200 bg-sky-50 hover:bg-sky-100 hover:border-sky-300 text-left text-base font-medium text-[#1a2744] active:scale-[0.99] transition-all whitespace-pre-wrap"
            >
              {it.text}
            </button>
          ))}
          ...
          <div className="flex flex-col gap-2">
            {order.map((id, i) => (
              <button
                ...
                disabled={submitting}
                onClick={() => remove(id)}
                className="min-h-[44px] px-4 py-3 rounded-2xl border-2 border-amber-300 bg-amber-50 hover:bg-amber-100 text-left text-base font-medium text-amber-900 flex items-center gap-3 transition-all whitespace-pre-wrap"
                title="Tap to remove"
              >
