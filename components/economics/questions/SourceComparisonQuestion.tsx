...
import type {
  ...
  EconomicsComparisonSource,
  EconomicsComparisonStatement,
  EconomicsProvenance,
} from "@/types/economics";

interface Props {
  sourceA: EconomicsComparisonSource;
  sourceB: EconomicsComparisonSource;
  statements: EconomicsComparisonStatement[];
  submitting: boolean;
  onSubmit: (payload: string) => void;
}

const ANSWERS: { id: EconomicsComparisonAnswer; label: string }[] = [
...
function ProvenanceLine({ provenance }: { provenance?: EconomicsProvenance }) {
...
function SourceCard({ letter, source }: { letter: "A" | "B"; source: EconomicsComparisonSource }) {
  return (
...
export default function SourceComparisonQuestion({
  ...
  const [picks, setPicks] = useState<Record<string, EconomicsComparisonAnswer>>({});
  const allPicked = statements.every((s) => picks[s.id]);

  function pick(stmtId: string, ans: EconomicsComparisonAnswer) {
