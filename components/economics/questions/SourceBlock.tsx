...
// SourceBlock — optional source rendered ABOVE any Economics mechanic.
...
import type { EconomicsProvenance, EconomicsSource } from "@/types/economics";

const ACCT_IMAGE_EXTS = ["png", "svg", "jpg", "jpeg", "webp"];

function ProvenanceBadge({ provenance }: { provenance?: EconomicsProvenance }) {
...
function SourceImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/economics/${imageKey}.${ACCT_IMAGE_EXTS[extIdx]}`}
        alt={alt}
...
export default function SourceBlock({ source }: { source: EconomicsSource }) {
