...
export default function SocialSciencesSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      ...
      try {
        const question = await fetchQuestionOrLocal<SocialSciencesSpGeneratedQuestion>({
          url: "/api/social-sciences-sp/generate-question",
          body: {
            skill_id: topicId,
            used_refs: used,
  ...
  const distinctAnswered = profile ? new Set(getSocialSciencesSpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
