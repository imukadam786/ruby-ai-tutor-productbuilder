...
export default function LifeOrientationSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      setSubmitting(true);
      try {
        const payload: LifeOrientationSpSubmitAnswerRequest = {
        ...
        const res = await apiFetch("/api/life-orientation-sp/submit-answer", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = (await res.json()) as LifeOrientationSpSubmitAnswerResponse;
        setResult(data);
        ...
        const workingProfile = profile
          ? markLifeOrientationSpQuestionUsed(profile, skillId, question.question_ref)
          : null;
        ...
        const distinctAnswered = workingProfile
          ? new Set(getLifeOrientationSpUsedRefs(workingProfile, skillId)).size
          : nextAttempts;
        ...
        if (runOver) {
          if (profile) {
            const finalProfile = recordLifeOrientationSpSkillResult(profile, skillId, {
              correct: nextCorrect,
          ...
          rewardEffortFloor("life-orientation-sp", skillId);
          if (didMaster) rewardSkillMastered("life-orientation-sp", skillId, profile?.id);
        ...
        console.error("[LifeOrientationSpSession] submit-answer failed:", err);
