...
import type {
  ...
  LifeOrientationSpGenerateQuestionResponse,
...
export default function LifeOrientationSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      ...
      try {
        const res = await apiFetch("/api/life-orientation-sp/generate-question", {
          method: "POST",
        ...
        const data = (await res.json()) as LifeOrientationSpGenerateQuestionResponse;
        if (!data.question) {
        ...
        <div className="max-w-2xl mx-auto px-5 sm:px-8 pt-4 pb-12 w-full space-y-5">
          ...
          {skillId && (
            <MasteryHeader
              ...
              distinctAnswered={profile ? new Set(getLifeOrientationSpUsedRefs(profile, skillId)).size : 0}
