...
// LifeOrientationSpSession — clone of GeographySession for the Natural Sciences
...
//     lib/life-orientation-sp-scoring.ts):
...
import lifeOrientationSpTreeData from "@/data/life-orientation-sp-skill-tree.json";
...
import { TOPIC_EXAMPLES } from "@/lib/life-orientation-sp-topic-feedback";
...
} from "@/lib/life-orientation-sp-selector";
import { fetchQuestionOrLocal } from "@/lib/offline/fetchQuestionOrLocal";
import { fetchAuthorisedGrade } from "@/lib/onboarding-reader";
import {
  getLifeOrientationSpUsedRefs,
  getOrCreateLifeOrientationSpProfile,
  hydrateLifeOrientationSpProfileFromSupabase,
  linkLifeOrientationSpProfileToAuth,
  loadLifeOrientationSpProfile,
  markLifeOrientationSpQuestionUsed,
  markLifeOrientationSpSkillInProgress,
  recordLifeOrientationSpSkillResult,
  saveLifeOrientationSpProfile,
...
import type {
  LifeOrientationSpBank,
  LifeOrientationSpBankQuestion,
  LifeOrientationSpGeneratedQuestion,
  LifeOrientationSpSkillTree,
  LifeOrientationSpStudentProfile,
  LifeOrientationSpSubmitAnswerRequest,
  LifeOrientationSpSubmitAnswerResponse,
} from "@/types/life-orientation-sp";

const tree = lifeOrientationSpTreeData as unknown as LifeOrientationSpSkillTree;
const bank = lifeOrientationSpBankData as unknown as LifeOrientationSpBank;
...
export default function LifeOrientationSpSession({ onBack }: { onBack?: () => void } = {}) {
  const [phase, setPhase] = useState<Phase>("tree");
  const [skillId, setSkillId] = useState<string | null>(null);
  const [question, setQuestion] = useState<LifeOrientationSpGeneratedQuestion | null>(null);
  const [result, setResult] = useState<LifeOrientationSpSubmitAnswerResponse | null>(null);
  // Feedback is scored client-side and shown instantly — no "Checking…" wait.
  const [submitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [profile, setProfile] = useState<LifeOrientationSpStudentProfile | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadLifeOrientationSpProfile()) {
        const restored = await hydrateLifeOrientationSpProfileFromSupabase();
        if (restored) saveLifeOrientationSpProfile(restored);
      }
      const p = getOrCreateLifeOrientationSpProfile(grade, name);
      void linkLifeOrientationSpProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[LifeOrientationSpSession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[LifeOrientationSpSession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: LifeOrientationSpStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      setError(null);
      const used = currentProfile ? getLifeOrientationSpUsedRefs(currentProfile, topicId) : [];
      ...
      try {
        ...
        const question = await fetchQuestionOrLocal<LifeOrientationSpGeneratedQuestion>({
          url: "/api/life-orientation-sp/generate-question",
          body: {
            skill_id: topicId,
            used_refs: used,
        ...
        console.error("[LifeOrientationSpSession] generate-question failed:", err);
  ...
  const handlePickTopic = useCallback(
    (topicId: string) => {
      ...
      if (profile) {
        const updated = markLifeOrientationSpSkillInProgress(profile, topicId);
        setProfile(updated);
      }
      trackSessionStarted({
        subject: "life-orientation-sp",
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_life_orientation_sp_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_life_orientation_sp_target_skill");
  ...
  const finalizeAttempt = useCallback(
    (data: LifeOrientationSpSubmitAnswerResponse, q: LifeOrientationSpGeneratedQuestion, sId: string) => {
      setResult(data);
      ...
      const workingProfile = profile
        ? markLifeOrientationSpQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getLifeOrientationSpUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordLifeOrientationSpSkillResult(profile, sId, {
            correct: nextCorrect,
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreLifeOrientationSpAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as LifeOrientationSpBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/life-orientation-sp/submit-answer", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          used_hint: false,
        } as LifeOrientationSpSubmitAnswerRequest),
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getLifeOrientationSpUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getLifeOrientationSpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
  ...
  return (
    ...
    >
      ...
      {question.context && (
        <div className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-900">
          {question.context}
        </div>
      )}
      ...
      {phase === "feedback" && result && (
        <FeedbackExplanation
          ...
          {...topicFeedbackProps({
            isCorrect: result.is_correct,
            subjectSlug: "life-orientation-sp",
      ...
      {error && (
        <p className="text-sm text-slate-600 bg-slate-50 rounded-xl px-3 py-2">{error}</p>
      )}
    </QuizShell>
  );
}
...
const EMS_IMAGE_EXTS = ["png", "svg", "jpg", "jpeg", "webp"];

function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  const [extIdx, setExtIdx] = useState(0);
  if (extIdx >= EMS_IMAGE_EXTS.length) {
    return (
      <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-4 text-center text-sm italic text-slate-700">
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/life-orientation-sp/${imageKey}.${EMS_IMAGE_EXTS[extIdx]}`}
        alt={alt}
        onError={() => setExtIdx((i) => i + 1)}
        className="max-h-80 rounded-2xl border border-slate-200 bg-white"
      />
    </div>
  );
}
...
interface AnswerDispatcherProps {
  question: LifeOrientationSpGeneratedQuestion;
  submitting: boolean;
...
function AnswerDispatcher({ question, submitting, onSubmit }: AnswerDispatcherProps) {
      ...
      <div className={`grid ${gridCols} gap-3`}>
        {options.map((opt) => (
          <button
            ...
            disabled={submitting}
            onClick={() => onSubmit(opt)}
            className="bg-slate-50 hover:bg-slate-100 border-2 border-slate-200 hover:border-slate-300 rounded-2xl px-5 py-5 text-left text-base sm:text-lg font-semibold text-[#1a2744] active:scale-95 transition-all whitespace-pre-line"
          >
            {opt}
          </button>
        ))}
  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-800">
