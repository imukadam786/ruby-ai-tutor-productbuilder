...
// TourismSession — clone of LifeSciencesSession with a wider answer
...
//     above the mechanic via components/tourism/questions/SourceBlock.

import { useCallback, useEffect, useRef, useState } from "react";
import { apiFetch } from "@/lib/fetch";
import { supabase } from "@/lib/supabase";
import tourismTreeData from "@/data/tourism-skill-tree.json";
...
import { TOPIC_EXAMPLES } from "@/lib/tourism-topic-feedback";
...
import {
  getTourismUsedRefs,
  getOrCreateTourismProfile,
  hydrateTourismProfileFromSupabase,
  linkTourismProfileToAuth,
  loadTourismProfile,
  markTourismQuestionUsed,
  markTourismSkillInProgress,
  recordTourismSkillResult,
  saveTourismProfile,
...
const tree = tourismTreeData as unknown as TourismSkillTree;
...
export default function TourismSession({ onBack }: { onBack?: () => void } = {}) {
  const [phase, setPhase] = useState<Phase>("tree");
  const [skillId, setSkillId] = useState<string | null>(null);
  const [question, setQuestion] = useState<TourismGeneratedQuestion | null>(null);
  const [result, setResult] = useState<TourismSubmitAnswerResponse | null>(null);
  // Feedback is scored client-side and shown instantly — no "Checking…" wait.
  const [submitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [profile, setProfile] = useState<TourismStudentProfile | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadTourismProfile()) {
        const restored = await hydrateTourismProfileFromSupabase();
        if (restored) saveTourismProfile(restored);
      }
      const p = getOrCreateTourismProfile(grade, name);
      void linkTourismProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[TourismSession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[TourismSession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: TourismStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      setError(null);
      const used = currentProfile ? getTourismUsedRefs(currentProfile, topicId) : [];
      ...
      try {
        ...
        console.error("[TourismSession] generate-question failed:", err);
  ...
  const handlePickTopic = useCallback(
    (topicId: string) => {
      ...
      if (profile) {
        const updated = markTourismSkillInProgress(profile, topicId);
        setProfile(updated);
      }
      trackSessionStarted({
        subject: "tourism",
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_tourism_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_tourism_target_skill");
  ...
  const finalizeAttempt = useCallback(
    (data: TourismSubmitAnswerResponse, q: TourismGeneratedQuestion, sId: string) => {
      setResult(data);

      const workingProfile = profile
        ? markTourismQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getTourismUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordTourismSkillResult(profile, sId, {
            correct: nextCorrect,
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreTourismAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as TourismBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/tourism/submit-answer", {
        method: "POST",
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getTourismUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getTourismUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
  ...
  return (
    ...
    >
      ...
      {question.context && (
        <div className="bg-cyan-50 border border-cyan-200 rounded-2xl px-4 py-3 text-sm text-cyan-900">
          {question.context}
        </div>
      )}
...
function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  if (extIdx >= HIS_IMAGE_EXTS.length) {
    return (
      <div className="rounded-2xl border-2 border-dashed border-cyan-300 bg-cyan-50 p-4 text-center text-sm italic text-cyan-700">
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        ...
        className="max-h-80 rounded-2xl border border-cyan-200 bg-white"
      />
    </div>
  );
}
...
interface AnswerDispatcherProps {
  question: TourismGeneratedQuestion;
  submitting: boolean;
      ...
      <div className={`grid ${gridCols} gap-3`}>
        {options.map((opt) => (
          <button
            ...
            disabled={submitting}
            onClick={() => onSubmit(opt)}
            className="bg-cyan-50 hover:bg-cyan-100 border-2 border-cyan-200 hover:border-cyan-300 rounded-2xl px-5 py-5 text-left text-base sm:text-lg font-semibold text-[#1a2744] active:scale-95 transition-all whitespace-pre-line"
          >
            {opt}
          </button>
        ))}
