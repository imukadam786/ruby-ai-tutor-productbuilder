...
export default function TourismSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      setSubmitting(true);
      try {
        const payload: TourismSubmitAnswerRequest = {
        ...
        const res = await apiFetch("/api/tourism/submit-answer", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = (await res.json()) as TourismSubmitAnswerResponse;
        setResult(data);
        ...
        const workingProfile = profile
          ? markTourismQuestionUsed(profile, skillId, question.question_ref)
          : null;
        ...
        const distinctAnswered = workingProfile
          ? new Set(getTourismUsedRefs(workingProfile, skillId)).size
          : nextAttempts;
        ...
        if (runOver) {
          if (profile) {
            const finalProfile = recordTourismSkillResult(profile, skillId, {
              correct: nextCorrect,
          ...
          rewardEffortFloor("tourism", skillId);
          if (didMaster) rewardSkillMastered("tourism", skillId, profile?.id);
        ...
        console.error("[TourismSession] submit-answer failed:", err);
              ...
              {phase === "feedback" && result && (
                ...
                >
                  ...
                  {recoveryHint && !result.is_correct && (
                    <p className="text-sm text-cyan-700 mt-3 bg-cyan-100 rounded-xl px-3 py-2">
                      💡 {recoveryHint}
                    </p>
                  )}
                </div>
