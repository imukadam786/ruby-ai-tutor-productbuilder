...
import type {
  ...
  HistoryComparisonSource,
  HistoryComparisonStatement,
  HistoryProvenance,
} from "@/types/history";

interface Props {
  sourceA: HistoryComparisonSource;
  sourceB: HistoryComparisonSource;
  statements: HistoryComparisonStatement[];
  submitting: boolean;
  onSubmit: (payload: string) => void;
}

const ANSWERS: { id: HistoryComparisonAnswer; label: string }[] = [
...
function ProvenanceLine({ provenance }: { provenance?: HistoryProvenance }) {
...
function SourceCard({ letter, source }: { letter: "A" | "B"; source: HistoryComparisonSource }) {
  return (
...
export default function SourceComparisonQuestion({
  ...
  const [picks, setPicks] = useState<Record<string, HistoryComparisonAnswer>>({});
  const allPicked = statements.every((s) => picks[s.id]);

  function pick(stmtId: string, ans: HistoryComparisonAnswer) {
