...
//     above the mechanic via components/history/questions/SourceBlock.

import { useCallback, useEffect, useRef, useState } from "react";
import { apiFetch } from "@/lib/fetch";
import { supabase } from "@/lib/supabase";
import historyTreeData from "@/data/history-skill-tree.json";
...
import { TOPIC_EXAMPLES } from "@/lib/history-topic-feedback";
...
import {
  getHistoryUsedRefs,
  getOrCreateHistoryProfile,
  hydrateHistoryProfileFromSupabase,
  linkHistoryProfileToAuth,
  loadHistoryProfile,
  markHistoryQuestionUsed,
  markHistorySkillInProgress,
  recordHistorySkillResult,
  saveHistoryProfile,
...
export default function HistorySession({ onBack }: { onBack?: () => void } = {}) {
  const [phase, setPhase] = useState<Phase>("tree");
  const [skillId, setSkillId] = useState<string | null>(null);
  const [question, setQuestion] = useState<HistoryGeneratedQuestion | null>(null);
  const [result, setResult] = useState<HistorySubmitAnswerResponse | null>(null);
  // Feedback is scored client-side and shown instantly — no "Checking…" wait.
  const [submitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [profile, setProfile] = useState<HistoryStudentProfile | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadHistoryProfile()) {
        const restored = await hydrateHistoryProfileFromSupabase();
        if (restored) saveHistoryProfile(restored);
      }
      const p = getOrCreateHistoryProfile(grade, name);
      void linkHistoryProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[HistorySession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[HistorySession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      ...
      try {
        ...
        console.error("[HistorySession] generate-question failed:", err);
  ...
  const finalizeAttempt = useCallback(
    (data: HistorySubmitAnswerResponse, q: HistoryGeneratedQuestion, sId: string) => {
      setResult(data);

      const workingProfile = profile
        ? markHistoryQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getHistoryUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordHistorySkillResult(profile, sId, {
            correct: nextCorrect,
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreHistoryAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as HistoryBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/history/submit-answer", {
        method: "POST",
  ...
  const distinctAnswered = profile ? new Set(getHistoryUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
...
function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/history/${imageKey}.${HIS_IMAGE_EXTS[extIdx]}`}
        alt={alt}
...
interface AnswerDispatcherProps {
  question: HistoryGeneratedQuestion;
  submitting: boolean;
