...
export default function HistorySkillTreeView({
  ...
  const progress = useMemo(() => {
    ...
    return getHistoryLevelProgress(ids, profile.skill_mastery);
  }, [level, profile]);
  ...
  const currentSkillId = useMemo<string | null>(() => {
    ...
    for (const tier of level.tiers) {
      for (const skill of tier.atomic_skills) {
        const status = getHistorySkillStatus(skill.id, profile);
  ...
  const tierMeta = useMemo(() => {
    ...
    return level.tiers.map((tier) => {
      const skills = tier.atomic_skills.map((s) => {
        const baseStatus = getHistorySkillStatus(s.id, profile) as SkillStatus;
    ...
    return (
      <div className="flex items-center justify-center h-full bg-[#F4F4F5] p-6 text-center">
        <p className="text-gray-600">
          More History grades are coming soon.
        </p>
      </div>
    );
  }
