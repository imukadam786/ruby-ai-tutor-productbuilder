...
export default function LifeSciencesSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: LifeSciencesStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_life-sciences_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_life-sciences_target_skill");
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getLifeSciencesUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getLifeSciencesUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
...
const LSC_IMAGE_EXTS = ["webp", "png", "svg", "jpg"];
