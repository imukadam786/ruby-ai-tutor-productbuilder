...
export default function BusinessStudiesSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: BusinessStudiesStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      ...
      try {
        ...
        const question = await fetchQuestionOrLocal<BusinessStudiesGeneratedQuestion>({
          url: "/api/business-studies/generate-question",
          body: {
            skill_id: topicId,
            used_refs: used,
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_business-studies_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_business-studies_target_skill");
  ...
  const finalizeAttempt = useCallback(
    (data: BusinessStudiesSubmitAnswerResponse, q: BusinessStudiesGeneratedQuestion, sId: string) => {
      setResult(data);

      const workingProfile = profile
        ? markBusinessStudiesQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getBusinessStudiesUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordBusinessStudiesSkillResult(profile, sId, {
            correct: nextCorrect,
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreBusinessStudiesAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as BusinessStudiesBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/business-studies/submit-answer", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          used_hint: false,
        } as BusinessStudiesSubmitAnswerRequest),
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getBusinessStudiesUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getBusinessStudiesUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
