...
// Mirror of components/life-sciences/LifeSciencesSkillTreeView.tsx for the
// FET BusinessStudies subject. Differences:
//   • Tiers = CAPS terms T1–T4 (not strands S1–S4). Emojis are neutral term
//     icons: 📜 ⏳ 🌍 ⚖️.
//   • Title row says "BusinessStudies · Grade X".
...
// Term code (second segment of the tier id, e.g. L10.T1 → "T1") → emoji.
...
function termCode(tierId: string): string {
...
export default function BusinessStudiesSkillTreeView({
  ...
  const progress = useMemo(() => {
    ...
    return getBusinessStudiesLevelProgress(ids, profile.skill_mastery);
  }, [level, profile]);
  ...
  const currentSkillId = useMemo<string | null>(() => {
    ...
    for (const tier of level.tiers) {
      for (const skill of tier.atomic_skills) {
        const status = getBusinessStudiesSkillStatus(skill.id, profile);
  ...
  const tierMeta = useMemo(() => {
    ...
    return level.tiers.map((tier) => {
      const skills = tier.atomic_skills.map((s) => {
        const baseStatus = getBusinessStudiesSkillStatus(s.id, profile) as SkillStatus;
    ...
    return (
      <div className="flex items-center justify-center h-full bg-[#F4F4F5] p-6 text-center">
        <p className="text-gray-600">
          More BusinessStudies grades are coming soon.
        </p>
      </div>
    );
  }
        ...
        <div className={`max-w-3xl mx-auto w-full ${compact ? "px-1 pt-1 pb-4" : "px-5 sm:px-8 pt-5 pb-12"}`}>
          ...
          <div className="mb-5">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1a2744]">
              BusinessStudies · Grade {seed.level}
            </h1>
            ...
            {seed.belowContent && (
              <p className="mt-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2">
                BusinessStudies starts in Grade {LOWEST_AVAILABLE_LEVEL}. Here&apos;s the Grade {LOWEST_AVAILABLE_LEVEL} content to get a head start.
              </p>
            )}
            {seed.beyondContent && (
              <p className="mt-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2">
