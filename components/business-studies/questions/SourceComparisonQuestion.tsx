...
import type {
  ...
  BusinessStudiesComparisonSource,
  BusinessStudiesComparisonStatement,
  BusinessStudiesProvenance,
} from "@/types/business-studies";

interface Props {
  sourceA: BusinessStudiesComparisonSource;
  sourceB: BusinessStudiesComparisonSource;
  statements: BusinessStudiesComparisonStatement[];
  submitting: boolean;
  onSubmit: (payload: string) => void;
}

const ANSWERS: { id: BusinessStudiesComparisonAnswer; label: string }[] = [
...
function ProvenanceLine({ provenance }: { provenance?: BusinessStudiesProvenance }) {
...
function SourceCard({ letter, source }: { letter: "A" | "B"; source: BusinessStudiesComparisonSource }) {
  return (
...
export default function SourceComparisonQuestion({
  ...
  const [picks, setPicks] = useState<Record<string, BusinessStudiesComparisonAnswer>>({});
  const allPicked = statements.every((s) => picks[s.id]);

  function pick(stmtId: string, ans: BusinessStudiesComparisonAnswer) {
