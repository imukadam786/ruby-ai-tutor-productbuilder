...
// CreativeArtsSpSession — clone of GeographySession for the Natural Sciences
...
//     lib/creative-arts-sp-scoring.ts):
...
import creativeArtsSpTreeData from "@/data/creative-arts-sp-skill-tree.json";
...
import { TOPIC_EXAMPLES } from "@/lib/creative-arts-sp-topic-feedback";
...
import {
  ...
  getOrCreateCreativeArtsSpProfile,
  hydrateCreativeArtsSpProfileFromSupabase,
  linkCreativeArtsSpProfileToAuth,
  loadCreativeArtsSpProfile,
  markCreativeArtsSpQuestionUsed,
  markCreativeArtsSpSkillInProgress,
  recordCreativeArtsSpSkillResult,
  saveCreativeArtsSpProfile,
...
const tree = creativeArtsSpTreeData as unknown as CreativeArtsSpSkillTree;
const bank = creativeArtsSpBankData as unknown as CreativeArtsSpBank;
...
export default function CreativeArtsSpSession({ onBack }: { onBack?: () => void } = {}) {
  const [phase, setPhase] = useState<Phase>("tree");
  const [skillId, setSkillId] = useState<string | null>(null);
  const [question, setQuestion] = useState<CreativeArtsSpGeneratedQuestion | null>(null);
  const [result, setResult] = useState<CreativeArtsSpSubmitAnswerResponse | null>(null);
  // Feedback is scored client-side and shown instantly — no "Checking…" wait.
  const [submitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [profile, setProfile] = useState<CreativeArtsSpStudentProfile | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadCreativeArtsSpProfile()) {
        const restored = await hydrateCreativeArtsSpProfileFromSupabase();
        if (restored) saveCreativeArtsSpProfile(restored);
      }
      const p = getOrCreateCreativeArtsSpProfile(grade, name);
      void linkCreativeArtsSpProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[CreativeArtsSpSession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[CreativeArtsSpSession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: CreativeArtsSpStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      setError(null);
      const used = currentProfile ? getCreativeArtsSpUsedRefs(currentProfile, topicId) : [];
      ...
      try {
        ...
        console.error("[CreativeArtsSpSession] generate-question failed:", err);
  ...
  const handlePickTopic = useCallback(
    (topicId: string) => {
      ...
      if (profile) {
        const updated = markCreativeArtsSpSkillInProgress(profile, topicId);
        setProfile(updated);
      }
      trackSessionStarted({
        subject: "creative-arts-sp",
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_creative_arts_sp_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_creative_arts_sp_target_skill");
  ...
  const finalizeAttempt = useCallback(
    (data: CreativeArtsSpSubmitAnswerResponse, q: CreativeArtsSpGeneratedQuestion, sId: string) => {
      setResult(data);
      ...
      const workingProfile = profile
        ? markCreativeArtsSpQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getCreativeArtsSpUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordCreativeArtsSpSkillResult(profile, sId, {
            correct: nextCorrect,
        ...
        rewardEffortFloor("creative-arts-sp", sId);
        if (didMaster) rewardSkillMastered("creative-arts-sp", sId);
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreCreativeArtsSpAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as CreativeArtsSpBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/creative-arts-sp/submit-answer", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          used_hint: false,
        } as CreativeArtsSpSubmitAnswerRequest),
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getCreativeArtsSpUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getCreativeArtsSpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
...
function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/creative-arts-sp/${imageKey}.${EMS_IMAGE_EXTS[extIdx]}`}
        alt={alt}
...
interface AnswerDispatcherProps {
  question: CreativeArtsSpGeneratedQuestion;
  submitting: boolean;
