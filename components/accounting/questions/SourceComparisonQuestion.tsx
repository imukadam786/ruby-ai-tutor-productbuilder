...
import type {
  ...
  AccountingComparisonSource,
  AccountingComparisonStatement,
  AccountingProvenance,
} from "@/types/accounting";

interface Props {
  sourceA: AccountingComparisonSource;
  sourceB: AccountingComparisonSource;
  statements: AccountingComparisonStatement[];
  submitting: boolean;
  onSubmit: (payload: string) => void;
}

const ANSWERS: { id: AccountingComparisonAnswer; label: string }[] = [
...
function ProvenanceLine({ provenance }: { provenance?: AccountingProvenance }) {
...
function SourceCard({ letter, source }: { letter: "A" | "B"; source: AccountingComparisonSource }) {
  return (
...
export default function SourceComparisonQuestion({
  ...
  const [picks, setPicks] = useState<Record<string, AccountingComparisonAnswer>>({});
  const allPicked = statements.every((s) => picks[s.id]);

  function pick(stmtId: string, ans: AccountingComparisonAnswer) {
