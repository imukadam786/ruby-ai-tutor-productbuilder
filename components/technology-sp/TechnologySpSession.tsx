...
// TechnologySpSession — clone of GeographySession for the Natural Sciences
...
import {
  getTechnologySpUsedRefs,
  getOrCreateTechnologySpProfile,
  hydrateTechnologySpProfileFromSupabase,
  linkTechnologySpProfileToAuth,
  loadTechnologySpProfile,
  markTechnologySpQuestionUsed,
  markTechnologySpSkillInProgress,
  recordTechnologySpSkillResult,
  saveTechnologySpProfile,
...
import type {
  ...
  TechnologySpSubmitAnswerRequest,
  TechnologySpSubmitAnswerResponse,
} from "@/types/technology-sp";

const tree = technologySpTreeData as unknown as TechnologySpSkillTree;
const bank = technologySpBankData as unknown as TechnologySpBank;
...
export default function TechnologySpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadTechnologySpProfile()) {
        const restored = await hydrateTechnologySpProfileFromSupabase();
        if (restored) saveTechnologySpProfile(restored);
      }
      const p = getOrCreateTechnologySpProfile(grade, name);
      void linkTechnologySpProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[TechnologySpSession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[TechnologySpSession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: TechnologySpStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      setError(null);
      const used = currentProfile ? getTechnologySpUsedRefs(currentProfile, topicId) : [];
      ...
      try {
        const res = await apiFetch("/api/technology-sp/generate-question", {
          method: "POST",
        ...
        const data = (await res.json()) as TechnologySpGenerateQuestionResponse;
        if (!data.question) {
        ...
        console.error("[TechnologySpSession] generate-question failed:", err);
        setError("Could not load a question. Please try again.");
        setPhase("feedback");
      }
    },
  ...
  const handlePickTopic = useCallback(
    (topicId: string) => {
      ...
      if (profile) {
        const updated = markTechnologySpSkillInProgress(profile, topicId);
        setProfile(updated);
      }
      trackSessionStarted({
        subject: "technology-sp",
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_technology_sp_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_technology_sp_target_skill");
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getTechnologySpUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
      ...
      <div className="relative flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-5 sm:px-8 pt-4 pb-12 w-full space-y-5">
          ...
          <MasteryHeader
            ...
            distinctAnswered={profile ? new Set(getTechnologySpUsedRefs(profile, skillId)).size : 0}
...
function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/technology-sp/${imageKey}.${EMS_IMAGE_EXTS[extIdx]}`}
        alt={alt}
...
interface AnswerDispatcherProps {
  question: TechnologySpGeneratedQuestion;
  submitting: boolean;
