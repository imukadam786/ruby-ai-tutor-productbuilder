...
export default function TechnologySpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      ...
      try {
        ...
        const question = await fetchQuestionOrLocal<TechnologySpGeneratedQuestion>({
          url: "/api/technology-sp/generate-question",
          body: {
            skill_id: topicId,
            used_refs: used,
  ...
  const distinctAnswered = profile ? new Set(getTechnologySpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
