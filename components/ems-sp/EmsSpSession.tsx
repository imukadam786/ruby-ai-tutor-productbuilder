...
export default function EmsSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadNextQuestion = useCallback(
    async (
      ...
      try {
        const res = await apiFetch("/api/ems-sp/generate-question", {
          method: "POST",
        ...
        const data = (await res.json()) as EmsSpGenerateQuestionResponse;
        if (!data.question) {
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      setSubmitting(true);
      try {
        const payload: EmsSpSubmitAnswerRequest = {
        ...
        const res = await apiFetch("/api/ems-sp/submit-answer", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = (await res.json()) as EmsSpSubmitAnswerResponse;
        setResult(data);
        ...
        const workingProfile = profile
          ? markEmsSpQuestionUsed(profile, skillId, question.question_ref)
          : null;
        ...
        const distinctAnswered = workingProfile
          ? new Set(getEmsSpUsedRefs(workingProfile, skillId)).size
          : nextAttempts;
        ...
        if (runOver) {
          if (profile) {
            const finalProfile = recordEmsSpSkillResult(profile, skillId, {
              correct: nextCorrect,
          ...
          rewardEffortFloor("ems-sp", skillId);
          if (didMaster) rewardSkillMastered("ems-sp", skillId, profile?.id);
        ...
        console.error("[EmsSpSession] submit-answer failed:", err);
                ...
                >
                  <p
                    className={`text-lg font-bold ${
                      result.is_correct ? "text-green-700" : "text-violet-700"
                    }`}
                  >
                    {result.is_correct ? "✓ " : "✗ "}
                    {result.feedback}
