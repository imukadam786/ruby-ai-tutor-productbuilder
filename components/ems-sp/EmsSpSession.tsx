...
import emsSpTreeData from "@/data/ems-sp-skill-tree.json";
...
import { TOPIC_EXAMPLES } from "@/lib/ems-sp-topic-feedback";
...
} from "@/lib/ems-sp-selector";
import { fetchQuestionOrLocal } from "@/lib/offline/fetchQuestionOrLocal";
import { fetchAuthorisedGrade } from "@/lib/onboarding-reader";
import {
  getEmsSpUsedRefs,
  getOrCreateEmsSpProfile,
  hydrateEmsSpProfileFromSupabase,
  linkEmsSpProfileToAuth,
  loadEmsSpProfile,
  markEmsSpQuestionUsed,
  markEmsSpSkillInProgress,
  recordEmsSpSkillResult,
  saveEmsSpProfile,
...
import type {
  EmsSpBank,
  EmsSpBankQuestion,
  EmsSpGeneratedQuestion,
  EmsSpSkillTree,
  EmsSpStudentProfile,
  EmsSpSubmitAnswerRequest,
  EmsSpSubmitAnswerResponse,
} from "@/types/ems-sp";

const tree = emsSpTreeData as unknown as EmsSpSkillTree;
const bank = emsSpBankData as unknown as EmsSpBank;
...
export default function EmsSpSession({ onBack }: { onBack?: () => void } = {}) {
  const [phase, setPhase] = useState<Phase>("tree");
  const [skillId, setSkillId] = useState<string | null>(null);
  const [question, setQuestion] = useState<EmsSpGeneratedQuestion | null>(null);
  const [result, setResult] = useState<EmsSpSubmitAnswerResponse | null>(null);
  // Feedback is scored client-side and shown instantly — no "Checking…" wait.
  const [submitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [profile, setProfile] = useState<EmsSpStudentProfile | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!loadEmsSpProfile()) {
        const restored = await hydrateEmsSpProfileFromSupabase();
        if (restored) saveEmsSpProfile(restored);
      }
      const p = getOrCreateEmsSpProfile(grade, name);
      void linkEmsSpProfileToAuth(p.id);
  ...
  const persistReport = useCallback(
    async (topicId: string, correct: number, attempts: number, didMaster: boolean) => {
      try {
        ...
        if (reportError) console.error("[EmsSpSession] student_reports insert failed:", reportError);
      } catch (err) {
        console.error("[EmsSpSession] persistReport failed:", err);
      }
    },
    [],
  );
  ...
  const loadNextQuestion = useCallback(
    async (
      topicId: string,
      currentProfile: EmsSpStudentProfile | null,
      sessionCorrect = 0,
      sessionAttempts = 0,
    ) => {
      setPhase("loading");
      setError(null);
      const used = currentProfile ? getEmsSpUsedRefs(currentProfile, topicId) : [];
      ...
      try {
        ...
        const question = await fetchQuestionOrLocal<EmsSpGeneratedQuestion>({
          url: "/api/ems-sp/generate-question",
          body: {
            skill_id: topicId,
            used_refs: used,
        ...
        console.error("[EmsSpSession] generate-question failed:", err);
  ...
  const handlePickTopic = useCallback(
    (topicId: string) => {
      ...
      if (profile) {
        const updated = markEmsSpSkillInProgress(profile, topicId);
        setProfile(updated);
      }
      trackSessionStarted({
        subject: "ems-sp",
  ...
  useEffect(() => {
    ...
    const target = sessionStorage.getItem("ruby_ems_sp_target_skill");
    if (!target) return;
    deepLinkConsumed.current = true;
    sessionStorage.removeItem("ruby_ems_sp_target_skill");
  ...
  const finalizeAttempt = useCallback(
    (data: EmsSpSubmitAnswerResponse, q: EmsSpGeneratedQuestion, sId: string) => {
      setResult(data);
      ...
      const workingProfile = profile
        ? markEmsSpQuestionUsed(profile, sId, q.question_ref)
        : null;
      ...
      const distinctAnswered = workingProfile
        ? new Set(getEmsSpUsedRefs(workingProfile, sId)).size
        : nextAttempts;
      ...
      if (runOver) {
        if (profile) {
          const finalProfile = recordEmsSpSkillResult(profile, sId, {
            correct: nextCorrect,
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      const { correct, score } = scoreEmsSpAnswer(
        question.input_type,
        rawAnswer,
        { ...question, expected: question.expected_answer } as unknown as EmsSpBankQuestion,
      );
      finalizeAttempt(
        {
          is_correct: correct,
      ...
      void apiFetch("/api/ems-sp/submit-answer", {
        method: "POST",
  ...
  if (phase === "mastered" && skillId) {
    ...
    const distinct = profile
      ? Math.min(new Set(getEmsSpUsedRefs(profile, skillId)).size, size)
      : 0;
    return (
  ...
  const distinctAnswered = profile ? new Set(getEmsSpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
  ...
  return (
    ...
    >
      ...
      {error && (
        <p className="text-sm text-violet-600 bg-violet-50 rounded-xl px-3 py-2">{error}</p>
      )}
    </QuizShell>
  );
}
...
function DiagramImage({ imageKey, alt }: { imageKey: string; alt: string }) {
  ...
  if (extIdx >= EMS_IMAGE_EXTS.length) {
    return (
      <div className="rounded-2xl border-2 border-dashed border-violet-300 bg-violet-50 p-4 text-center text-sm italic text-violet-700">
  ...
  return (
    <div className="flex justify-center">
      ...
      <img
        src={`/ems-sp/${imageKey}.${EMS_IMAGE_EXTS[extIdx]}`}
        alt={alt}
        onError={() => setExtIdx((i) => i + 1)}
        className="max-h-80 rounded-2xl border border-violet-200 bg-white"
      />
    </div>
  );
}
...
interface AnswerDispatcherProps {
  question: EmsSpGeneratedQuestion;
  submitting: boolean;
...
function AnswerDispatcher({ question, submitting, onSubmit }: AnswerDispatcherProps) {
      ...
      <div className={`grid ${gridCols} gap-3`}>
        {options.map((opt) => (
          <button
            ...
            disabled={submitting}
            onClick={() => onSubmit(opt)}
            className="bg-violet-50 hover:bg-violet-100 border-2 border-violet-200 hover:border-violet-300 rounded-2xl px-5 py-5 text-left text-base sm:text-lg font-semibold text-[#1a2744] active:scale-95 transition-all whitespace-pre-line"
          >
            {opt}
          </button>
        ))}
  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-violet-300 bg-violet-50 p-4 text-sm text-violet-800">
