      ...
      <div className="flex flex-col h-full bg-gradient-to-br from-blue-50 to-indigo-100 items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-sm w-full text-center space-y-5">
          ...
          <div>
            ...
            <p className="text-blue-600 font-medium mt-2 text-sm">No pressure — just do your best 😊</p>
          </div>
      ...
      <div className="flex flex-col h-full bg-gradient-to-br from-blue-50 to-indigo-100 overflow-y-auto">
        <div className="max-w-lg mx-auto w-full p-4 sm:p-6 space-y-4">
          <div className="bg-white rounded-3xl shadow-md p-6 text-center space-y-3">
            ...
            <p className="text-gray-500 text-sm">I&apos;ve found your perfect starting point.</p>
            {autoCount > 0 && (
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3">
              <p className="text-blue-400 text-xs font-semibold uppercase tracking-wide mb-1">You&apos;re starting at</p>
            ...
            {!placementResult.hardGatePassed && (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-left">
                <p className="text-amber-700 text-sm">🔑 We&apos;ll build your spelling foundations first — it&apos;s the key to great reading!</p>
              </div>
            )}
          </div>

          <div className="bg-white rounded-3xl shadow-md p-5">
            <h3 className="font-bold text-gray-700 text-xs uppercase tracking-wide mb-3">Your Skill Map</h3>
                ...
                return (
                  <div key={g.label}>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      {mc > 0 && <span className="text-xs text-green-600 font-semibold">{mc}/{g.ids.length}</span>}
                    </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-5">
        <div className="max-w-md mx-auto space-y-4">
          ...
          <div className="flex items-center gap-2">
            <span className="bg-indigo-100 text-indigo-700 text-xs font-semibold px-3 py-1 rounded-full">{task.domain}</span>
            {speaking && <span className="text-xs text-blue-400 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse" />Reading aloud…</span>}
          </div>
            ...
            {isFlashTask && (
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8 text-center min-h-[100px] flex items-center justify-center">
                {flashVisible ? (
                  ...
                  <p className="text-gray-400 text-sm">Get ready… the word is about to appear!</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-md p-5 space-y-4">
              ...
              <div className={`grid gap-3 ${task.choices.length === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
                {task.choices.map((c) => (
                  <button
                    ...
                    className={`px-4 py-4 rounded-2xl border-2 text-sm font-semibold text-left transition-all active:scale-95 ${
            ...
            {task.answerMode === "voice" && (
              <div className="space-y-4">
                {task.voiceHint && <p className="text-xs text-gray-400 text-center">{task.voiceHint}</p>}
                ...
                {transcript && (
                  <div className="bg-green-50 border border-green-200 rounded-2xl px-4 py-3 text-center">
                    <p className="text-xs text-green-500 font-medium mb-1">I heard:</p>
