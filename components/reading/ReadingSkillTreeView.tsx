...
interface ReadingSkillTreeViewProps {
  ...
  /** When provided, makes the active skill tile tappable to resume. */
  onContinue?: () => void;
...
export default function ReadingSkillTreeView({ profile, onReplaySkill, onContinue, onBack, compact }: ReadingSkillTreeViewProps) {
  ...
  function getExtendedStatus(skillId: string): SkillTreeStatus {
    ...
    const base = getReadingSkillStatus(skillId, profile!) as SkillTreeStatus;
  ...
  const levels: TreeLevel[] = useMemo(() => {
    ...
    const source = showAllLevels
      ? treeData.levels
      : treeData.levels.filter((l) => l.id === currentLevelId);
    return source.map((level): TreeLevel => {
      ...
      if (isHardGateLevel && !hardGatePassed) {
        banner = { tone: "amber", icon: "🔒", text: "Hard Gate — encoding mastery required before advancing" };
      } else if (isHardGateLevel && hardGatePassed && profile.placementCompleted) {
        banner = { tone: "green", icon: "✅", text: "Hard Gate passed" };
      }

      const preBarrier =
        level.id === 2 && !hardGatePassed && profile.placementCompleted
          ? { text: "Hard Gate — master encoding skills to unlock advanced phonics" }
          : undefined;
