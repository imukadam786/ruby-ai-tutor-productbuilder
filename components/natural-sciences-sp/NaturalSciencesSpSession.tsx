...
export default function NaturalSciencesSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const handleSubmit = useCallback(
    async (rawAnswer: string) => {
      ...
      setSubmitting(true);
      try {
        const payload: NaturalSciencesSpSubmitAnswerRequest = {
        ...
        const res = await apiFetch("/api/natural-sciences-sp/submit-answer", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = (await res.json()) as NaturalSciencesSpSubmitAnswerResponse;
        setResult(data);
        ...
        const workingProfile = profile
          ? markNaturalSciencesSpQuestionUsed(profile, skillId, question.question_ref)
          : null;
        ...
        const distinctAnswered = workingProfile
          ? new Set(getNaturalSciencesSpUsedRefs(workingProfile, skillId)).size
          : nextAttempts;
        ...
        if (runOver) {
          if (profile) {
            const finalProfile = recordNaturalSciencesSpSkillResult(profile, skillId, {
              correct: nextCorrect,
        ...
        console.error("[NaturalSciencesSpSession] submit-answer failed:", err);
              ...
              {phase === "feedback" && result && (
                ...
                >
                  ...
                  {recoveryHint && !result.is_correct && (
                    <p className="text-sm text-rose-700 mt-3 bg-rose-100 rounded-xl px-3 py-2">
                      💡 {recoveryHint}
                    </p>
                  )}
                </div>
