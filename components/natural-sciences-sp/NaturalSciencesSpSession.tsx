...
import { TOPIC_EXAMPLES } from "@/lib/natural-sciences-sp-topic-feedback";
...
export default function NaturalSciencesSpSession({ onBack }: { onBack?: () => void } = {}) {
  ...
  const distinctAnswered = profile ? new Set(getNaturalSciencesSpUsedRefs(profile, skillId)).size : 0;
  const required = requiredCount(skillId);
    ...
    >
      ...
      {phase === "feedback" && result && (
        <FeedbackExplanation
          ...
          {...topicFeedbackProps({
            isCorrect: result.is_correct,
            subjectSlug: "natural-sciences-sp",
