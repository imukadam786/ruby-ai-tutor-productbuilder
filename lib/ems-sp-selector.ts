 ...
 * "EMS.G7.LL.A1"). Each topic is 1:1 with a skill — the topic_id IS the
 * skill_id. Content-subject rule: 20-item pool per topic, 60% pass mark,
 * no timer.
 *
 * Per-mechanic `expected` payload handling lives in
 * lib/ems-sp-scoring.ts. This file just shuffles items and converts
...
export function getTopic(topicId: string): EmsSpTopic | null {
 ...
 *
 * @param topicId        e.g. "EMS.G7.LL.A1"
...
export function listTopicsForLevel(
  level: number,
): Array<{ id: string; topic: EmsSpTopic }> {
