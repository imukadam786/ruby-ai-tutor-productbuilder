// ─── Creative Arts SP grade → entry-level map ───────────────────────────────────
// Onboarding captures a grade (R, 1–12). Creative Arts is a Senior-Phase subject:
// Grades 7, 8 and 9 only. CAPS Creative Arts covers two art forms here — Music
// and Visual Arts (the two with rich tap-native theory; Dance and Drama are
// movement/performance-based and excluded). Every skill unlocks once the grade
// is selected (every pool is gate "NONE") — the UI just surfaces the "current"
// skill (next unfinished) and those after it.
//
...
//
// Creative Arts is NOT placement-tested. The learner self-reports grade in
// onboarding. Each grade opens on its first Music literacy skill (CAPS Term 1
// foundation).

export const HIGHEST_AVAILABLE_LEVEL = 9;
export const LOWEST_AVAILABLE_LEVEL = 7;

export interface CreativeArtsSpEntrySeed {
...
// Grade → tree level. Senior-Phase Creative Arts only.
...
// Each grade's CAPS-order entry skill — the first Music literacy skill opens
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/creative-arts-sp-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): CreativeArtsSpEntrySeed {
