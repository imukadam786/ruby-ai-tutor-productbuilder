/**
 * Accounting question selector. Direct mirror of lib/life-sciences-selector.ts.
 *
 * Bank shape: items live under `topics` (keyed by atomic skill id, e.g.
 * "ACC.G10.T1.WIDER.A1"). Each topic is 1:1 with a skill — the topic_id IS
 * the skill_id. FET rule: 20-item pool per topic, 60% pass mark, no timer.
 *
 * Per-mechanic `expected` payload handling lives in lib/accounting-scoring.ts.
...
export function getTopic(topicId: string): AccountingTopic | null {
 ...
 *
 * @param topicId        e.g. "ACC.G10.T1.WIDER.A1"
...
export function bankQuestionToGenerated(
  q: AccountingBankQuestion,
  skillId: string,
): AccountingGeneratedQuestion {
  return {
    id: `q_acc_${skillId}_${q.ref}_${Date.now()}`,
    skill_id: skillId,
    question_ref: q.ref,
...
export function listTopicsForLevel(
  level: number,
): Array<{ id: string; topic: AccountingTopic }> {
