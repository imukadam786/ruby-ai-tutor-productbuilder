...
import { seedForGrade } from "@/lib/life-orientation-sp-grade-map";
import type {
  LifeOrientationSpAtomicSkill,
  LifeOrientationSpSkillMastery,
  LifeOrientationSpSkillTree,
  LifeOrientationSpStudentProfile,
} from "@/types/life-orientation-sp";

const STORAGE_KEY = "life-orientation-sp-profile-v1";
const SUBJECT = "life-orientation-sp";

const tree = lifeOrientationSpSkillTreeData as unknown as LifeOrientationSpSkillTree;

// ─── Load / Save (localStorage + Supabase mirror) ─────────────────────────────

export function loadLifeOrientationSpProfile(): LifeOrientationSpStudentProfile | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as LifeOrientationSpStudentProfile;
  } catch {
    return null;
  }
}

export function saveLifeOrientationSpProfile(profile: LifeOrientationSpStudentProfile): void {
...
export async function linkLifeOrientationSpProfileToAuth(profileId: string): Promise<void> {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    const user = session?.user;
    if (!user) return;
...
export async function hydrateLifeOrientationSpProfileFromSupabase(): Promise<LifeOrientationSpStudentProfile | null> {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    const user = session?.user;
    if (!user) return null;
    ...
    return data.profile_data as unknown as LifeOrientationSpStudentProfile;
  } catch {
    return null;
  }
}

export function createLifeOrientationSpProfile(name: string, grade: number): LifeOrientationSpStudentProfile {
  const seed = seedForGrade(grade);
  const profile: LifeOrientationSpStudentProfile = {
    id: typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `lo_${Date.now()}`,
    name,
    grade,
  ...
  saveLifeOrientationSpProfile(profile);
  return profile;
}

export function getOrCreateLifeOrientationSpProfile(grade: number, name = "Learner"): LifeOrientationSpStudentProfile {
  return loadLifeOrientationSpProfile() ?? createLifeOrientationSpProfile(name, grade);
}

// ─── Tree helpers ─────────────────────────────────────────────────────────────

export function getLifeOrientationSpSkillById(skillId: string): LifeOrientationSpAtomicSkill | null {
...
export function getLifeOrientationSpLevelById(levelId: number) {
  return tree.levels.find((l) => l.id === levelId) || null;
}

export function getLifeOrientationSpTierById(tierId: string) {
  for (const level of tree.levels) {
...
export function friendlyLifeOrientationSpSkillName(skillId: string | null | undefined): string {
  if (!skillId) return "your starting topic";
  return getLifeOrientationSpSkillById(skillId)?.title ?? "your starting topic";
}

// ─── Mastery / status ─────────────────────────────────────────────────────────

export function initLifeOrientationSpSkillMastery(skillId: string): LifeOrientationSpSkillMastery {
  return {
    skill_id: skillId,
    status: "available",
    correct_count: 0,
 ...
 */
export function getLifeOrientationSpSkillStatus(
  skillId: string,
  ...
  const skill = getLifeOrientationSpSkillById(skillId);
...
export function markLifeOrientationSpSkillInProgress(
  profile: LifeOrientationSpStudentProfile,
  skillId: string,
): LifeOrientationSpStudentProfile {
  const existing = profile.skill_mastery[skillId];
  if (existing?.status === "mastered" || existing?.status === "in_progress") return profile;
  const updated: LifeOrientationSpStudentProfile = {
    ...profile,
    skill_mastery: {
      ...profile.skill_mastery,
      [skillId]: {
        ...(existing ?? initLifeOrientationSpSkillMastery(skillId)),
        status: "in_progress",
        last_attempted: new Date().toISOString(),
      },
    },
  ...
  saveLifeOrientationSpProfile(updated);
  return updated;
}
 ...
 */
export function recordLifeOrientationSpSkillResult(
  profile: LifeOrientationSpStudentProfile,
  skillId: string,
  result: { correct: number; attempts: number; mastered: boolean },
): LifeOrientationSpStudentProfile {
  const existing = profile.skill_mastery[skillId] ?? initLifeOrientationSpSkillMastery(skillId);
...
export function getLifeOrientationSpLevelProgress(
  skillIds: string[],
  masteryMap: Record<string, LifeOrientationSpSkillMastery>,
): number {
...
export function getLifeOrientationSpUsedRefs(
  profile: LifeOrientationSpStudentProfile,
  skillId: string,
): string[] {
...
export function markLifeOrientationSpQuestionUsed(
  profile: LifeOrientationSpStudentProfile,
  skillId: string,
  ref: string,
): LifeOrientationSpStudentProfile {
