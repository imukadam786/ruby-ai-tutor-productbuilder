 ...
 * "NSSP.G7.LL.A1"). Each topic is 1:1 with a skill — the topic_id IS the
 * skill_id. Content-subject rule: 20-item pool per topic, 60% pass mark,
 * no timer.
 *
 * Per-mechanic `expected` payload handling lives in
 * lib/natural-sciences-sp-scoring.ts. This file just shuffles items and converts
...
export function getTopic(topicId: string): NaturalSciencesSpTopic | null {
 ...
 *
 * @param topicId        e.g. "NSSP.G7.LL.A1"
...
export function selectQuestion(
  ...
  _skillId = "",
  abilityLevel?: number,
): NaturalSciencesSpBankQuestion | null {
...
export function bankQuestionToGenerated(
  q: NaturalSciencesSpBankQuestion,
  skillId: string,
): NaturalSciencesSpGeneratedQuestion {
  return {
    id: `q_nssp_${skillId}_${q.ref}_${Date.now()}`,
    skill_id: skillId,
    question_ref: q.ref,
...
export function listTopicsForLevel(
  level: number,
): Array<{ id: string; topic: NaturalSciencesSpTopic }> {
