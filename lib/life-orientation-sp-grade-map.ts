// ─── Life Orientation SP grade → entry-level map ───────────────────────────────────
// Onboarding captures a grade (R, 1–12). Life Orientation is a Senior-Phase subject:
// Grades 7, 8 and 9 only. CAPS sequences content by topic (Development of the
// self in society, Health & environmental responsibility, Constitutional rights
// & responsibilities, World of work) and term, but every skill unlocks once the
// grade is selected (every pool is gate "NONE") — the UI just surfaces the
// "current" skill (next unfinished) and those after it. Physical Education is
// excluded (movement-based, cannot be tapped).
//
...
//
// Life Orientation is NOT placement-tested. The learner self-reports grade in
// onboarding. Each grade opens on its first "Development of the self" skill (the
// CAPS Term 1 foundation).

export const HIGHEST_AVAILABLE_LEVEL = 9;
export const LOWEST_AVAILABLE_LEVEL = 7;

export interface LifeOrientationSpEntrySeed {
...
// Grade → tree level. Senior-Phase Life Orientation only.
...
// Each grade's CAPS-order entry skill — the first "Development of the self in
// society" skill opens each grade. Empty string = resolve at runtime by reading
// the first skill from the tree.
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/life-orientation-sp-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): LifeOrientationSpEntrySeed {
