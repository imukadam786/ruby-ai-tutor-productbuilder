...
//
// Built levels: L1–L6 = Grades 1–6, and L10–L12 = Grades 10–12 (CAPS FET FAL).
// NOTE the GAP at Grades 7–9 (Senior Phase FAL) — not yet authored. For any grade
// with no built content we fall back to the nearest available level AT OR BELOW
// the grade and flag `beyondContent` so callers can surface a "more grades coming"
// state. Grade R is treated as Grade 1 entry (FAL formally starts in Grade 1).

// Levels that actually have authored tree + bank content, ascending.
const AVAILABLE_LEVELS = [1, 2, 3, 4, 5, 6, 10, 11, 12] as const;

export const HIGHEST_AVAILABLE_LEVEL = 12; // Gr 1-6 + Gr 10-12 authored
...
export function seedForGrade(grade: string | number | null | undefined): AfrikaansEntrySeed {
  ...
  // No built content (Senior Phase 7–9 gap, or FET 11–12 not yet built):
