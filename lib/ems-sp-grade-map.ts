// ─── Economic & Management Sciences SP grade → entry-level map ────────────────
// Onboarding captures a grade (R, 1–12). EMS SP is a Senior-Phase subject:
// Grades 7, 8 and 9 only. CAPS sequences topics by strand (The Economy,
// Financial Literacy, Entrepreneurship) and term, but they all unlock once the
// grade is selected (every pool is gate "NONE") — the UI just surfaces the
// "current" topic (next unfinished) and the following ones.
//
// Content-subject rule: no timer, 60% pass mark per pool (same as Social
// Sciences SP / Natural Sciences SP).
//
// EMS SP is NOT placement-tested. The learner self-reports grade in onboarding.
// Each grade opens on its first Economy topic (CAPS Term 1).

export const HIGHEST_AVAILABLE_LEVEL = 9;
export const LOWEST_AVAILABLE_LEVEL = 7;

export interface EmsSpEntrySeed {
...
// Grade → tree level. Senior-Phase EMS only.
...
// Each grade's CAPS-order entry skill — the first Economy topic (Term 1) opens
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/ems-sp-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): EmsSpEntrySeed {
