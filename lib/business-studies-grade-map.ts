// ─── BusinessStudies grade → entry-level map ──────────────────────────────────────────
// Onboarding captures a grade (R, 1–12). BusinessStudies is a FET subject:
...
//
// BusinessStudies is NOT placement-tested. The learner self-reports grade in
...
export interface BusinessStudiesEntrySeed {
...
// Grade → tree level. FET BusinessStudies only.
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/business-studies-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): BusinessStudiesEntrySeed {
