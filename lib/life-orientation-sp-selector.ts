/**
 * Natural Sciences SP question selector. Direct mirror of
 * lib/geography-selector.ts.
 *
 * Bank shape: items live under `topics` (keyed by atomic skill id, e.g.
 * "LO.G7.LL.A1"). Each topic is 1:1 with a skill — the topic_id IS the
 * skill_id. Content-subject rule: 20-item pool per topic, 60% pass mark,
 * no timer.
 *
 * Per-mechanic `expected` payload handling lives in
 * lib/life-orientation-sp-scoring.ts. This file just shuffles items and converts
 * bank → generated shape.
 */
...
export function getTopic(topicId: string): LifeOrientationSpTopic | null {
...
export function selectQuestion(
  ...
  _skillId = "",
  abilityLevel?: number,
): LifeOrientationSpBankQuestion | null {
...
export function bankQuestionToGenerated(
  q: LifeOrientationSpBankQuestion,
  skillId: string,
): LifeOrientationSpGeneratedQuestion {
  return {
    id: `q_lo_${skillId}_${q.ref}_${Date.now()}`,
    skill_id: skillId,
    question_ref: q.ref,
...
export function listTopicsForLevel(
  level: number,
): Array<{ id: string; topic: LifeOrientationSpTopic }> {
