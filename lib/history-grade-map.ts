// ─── History grade → entry-level map ──────────────────────────────────────────
// Onboarding captures a grade (R, 1–12). History is a FET subject:
...
//
// History is NOT placement-tested. The learner self-reports grade in
...
export interface HistoryEntrySeed {
...
// Grade → tree level. FET History only.
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/history-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): HistoryEntrySeed {
