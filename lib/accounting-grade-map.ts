// ─── Accounting grade → entry-level map ──────────────────────────────────────────
// Onboarding captures a grade (R, 1–12). Accounting is a FET subject:
...
//
// Accounting is NOT placement-tested. The learner self-reports grade in
...
export interface AccountingEntrySeed {
...
// Grade → tree level. FET Accounting only.
...
const ENTRY_SKILL: Record<number, string> = {
  10: "ACC.G10.T1.INFORMAL.A1",
  11: "ACC.G11.T1.ETHICS.A1",
  12: "ACC.G12.T1.COCONCEPTS.A1",
};
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/accounting-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): AccountingEntrySeed {
