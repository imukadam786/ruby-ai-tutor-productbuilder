...
import type {
  ...
  TourismHighlightTarget,
  TourismInputType,
  TourismParagraphSlot,
  TourismSortBucketItem,
} from "@/types/tourism";
...
function scoreSortBuckets(studentAnswer: string, question: TourismBankQuestion): ScoreResult {
  const items: TourismSortBucketItem[] = question.items ?? [];
...
function scoreHighlightSource(studentAnswer: string, question: TourismBankQuestion): ScoreResult {
  const targets: TourismHighlightTarget[] = question.targets ?? [];
...
function scoreArgumentBuilder(studentAnswer: string, question: TourismBankQuestion): ScoreResult {
...
function scoreParagraphTemplate(studentAnswer: string, question: TourismBankQuestion): ScoreResult {
  const template: TourismParagraphSlot[] = question.template ?? [];
...
function scoreSourceComparison(studentAnswer: string, question: TourismBankQuestion): ScoreResult {
  const statements = question.statements ?? [];
  if (statements.length === 0) return FAIL;
  const answer = safeJsonParse<Record<string, TourismComparisonAnswer>>(studentAnswer);
  if (!answer) return FAIL;
...
export function scoreTourismAnswer(
  inputType: TourismInputType,
  studentAnswer: string,
  question: TourismBankQuestion,
): ScoreResult {
