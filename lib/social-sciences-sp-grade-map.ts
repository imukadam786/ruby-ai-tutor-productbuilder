// ─── Social Sciences SP grade → entry-level map ───────────────────────────────
// Onboarding captures a grade (R, 1–12). Social Sciences SP is a Senior-Phase
// subject: Grades 7, 8 and 9 only. CAPS sequences topics by discipline (History,
// Geography) and term, but they all unlock once the grade is selected (every
// pool is gate "NONE") — the UI just surfaces the "current" topic (next
// unfinished) and the following ones.
//
// Content-subject rule: no timer, 60% pass mark per pool (same as History /
// Geography / Natural Sciences SP).
//
// Social Sciences SP is NOT placement-tested. The learner self-reports grade in
// onboarding. Each grade opens on its first History topic.

export const HIGHEST_AVAILABLE_LEVEL = 9;
export const LOWEST_AVAILABLE_LEVEL = 7;

export interface SocialSciencesSpEntrySeed {
...
// Grade → tree level. Senior-Phase Social Sciences only.
...
// Each grade's CAPS-order entry skill — the first History topic (Term 1) opens
...
function firstSkillIdForLevel(level: number): string {
  const tree = require("../data/social-sciences-sp-skill-tree.json");
...
export function seedForGrade(grade: string | number | null | undefined): SocialSciencesSpEntrySeed {
