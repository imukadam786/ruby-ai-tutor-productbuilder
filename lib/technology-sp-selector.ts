 ...
 * "TECH.G7.LL.A1"). Each topic is 1:1 with a skill — the topic_id IS the
 * skill_id. Content-subject rule: 20-item pool per topic, 60% pass mark,
 * no timer.
 *
 * Per-mechanic `expected` payload handling lives in
 * lib/technology-sp-scoring.ts. This file just shuffles items and converts
...
export function getTopic(topicId: string): TechnologySpTopic | null {
 ...
 *
 * @param topicId        e.g. "TECH.G7.LL.A1"
...
export function bankQuestionToGenerated(
  q: TechnologySpBankQuestion,
  skillId: string,
): TechnologySpGeneratedQuestion {
  return {
    id: `q_ems_${skillId}_${q.ref}_${Date.now()}`,
    skill_id: skillId,
    question_ref: q.ref,
...
export function listTopicsForLevel(
  level: number,
): Array<{ id: string; topic: TechnologySpTopic }> {
