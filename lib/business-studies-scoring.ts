...
import type {
  BusinessStudiesBankQuestion,
  BusinessStudiesComparisonAnswer,
  BusinessStudiesHighlightTarget,
  BusinessStudiesInputType,
  BusinessStudiesParagraphSlot,
  BusinessStudiesSortBucketItem,
...
function scoreSortBuckets(studentAnswer: string, question: BusinessStudiesBankQuestion): ScoreResult {
  const items: BusinessStudiesSortBucketItem[] = question.items ?? [];
...
function scoreHighlightSource(studentAnswer: string, question: BusinessStudiesBankQuestion): ScoreResult {
  const targets: BusinessStudiesHighlightTarget[] = question.targets ?? [];
...
function scoreArgumentBuilder(studentAnswer: string, question: BusinessStudiesBankQuestion): ScoreResult {
...
function scoreParagraphTemplate(studentAnswer: string, question: BusinessStudiesBankQuestion): ScoreResult {
  const template: BusinessStudiesParagraphSlot[] = question.template ?? [];
...
function scoreSourceComparison(studentAnswer: string, question: BusinessStudiesBankQuestion): ScoreResult {
  const statements = question.statements ?? [];
  if (statements.length === 0) return FAIL;
  const answer = safeJsonParse<Record<string, BusinessStudiesComparisonAnswer>>(studentAnswer);
  if (!answer) return FAIL;
...
export function scoreBusinessStudiesAnswer(
  inputType: BusinessStudiesInputType,
  studentAnswer: string,
  question: BusinessStudiesBankQuestion,
): ScoreResult {
