...
//     The topic id IS the atomic skill id (e.g. "TECH.G7.LL.A1").
//
...
export type TechnologySpInputType =
  | "choice"
  | "true-false"
  | "cloze"
  | "scenario"
...
export type TechnologySpSourceType = "text" | "image" | "map" | "cartoon";

export interface TechnologySpProvenance {
  author?: string;
  date?: string;
  origin?: string;
...
export interface TechnologySpDataBlock {
...
export interface TechnologySpBankQuestion {
   ...
   *  public/technology-sp/<key>.{png|svg|jpg|webp} with a text fallback. */
  image_refs?: string[];

  // Optional `source` field — renders above the mechanic.
  source?: TechnologySpSource;

  // Per-mechanic payloads. Exactly one set is populated per question.
  data?: TechnologySpDataBlock;       // data-interpret

  buckets?: TechnologySpBucket[];     // sort-buckets
  items?: TechnologySpItem[];         // sort-buckets + sequence
  ...
  targets?: TechnologySpHighlightTarget[];
...
export type TechnologySpGate = "NONE";

/** One bank entry. Keyed in the bank by the atomic skill id
 *  (e.g. "TECH.G7.LL.A1"). */
export interface TechnologySpTopic {
  title: string;
  description?: string;
  grade: number;
  strand: string;
  skill_ids: string[];
  gate: TechnologySpGate;
  ...
  questions: TechnologySpBankQuestion[];
}

export interface TechnologySpBank {
  version: string;
  subject: "technology-sp";
  description?: string;
  /** key = atomic skill id (e.g. "TECH.G7.LL.A1"). */
  topics: Record<string, TechnologySpTopic>;
}

// ─── Skill Tree ───────────────────────────────────────────────────────────────

export interface TechnologySpErrorSignature {
  type: string;
  description: string;
  example: string;
}

export interface TechnologySpAtomicSkill {
  id: string;
  ...
  templates: string[];
  error_signatures: TechnologySpErrorSignature[];
  recovery_strategy: string;
  mastery_criteria: {
...
export interface TechnologySpTier {
  id: string;                      // "L7.LL"
  title: string;                   // "Term 1 — Life & Living"
  description?: string;
  atomic_skills: TechnologySpAtomicSkill[];
}

export interface TechnologySpLevel {
  id: number;
  grade: number;
  title: string;
  description: string;
  tiers: TechnologySpTier[];
}

export interface TechnologySpSkillTree {
  version: string;
  subject: "technology-sp";
  description?: string;
  grades_covered?: number[];
  levels: TechnologySpLevel[];
}

// ─── Student Model ────────────────────────────────────────────────────────────

export type TechnologySpMasteryStatus =
  | "locked"
  | "available"
  | "in_progress"
  | "mastered"
...
export interface TechnologySpSkillAttempt {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: TechnologySpInputType;
...
export interface TechnologySpSkillMastery {
  skill_id: string;
  status: TechnologySpMasteryStatus;
  correct_count: number;
  attempt_count: number;
  formats_used: TechnologySpInputType[];
  last_attempted: string;
  mastered_at?: string;
  last_reviewed_at?: string;
}

export interface TechnologySpStudentProfile {
  id: string;
  name: string;
  grade: number;
  current_level: number;
  current_skill_id: string;
  skill_mastery: Record<string, TechnologySpSkillMastery>;
...
export interface TechnologySpGeneratedQuestion {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: TechnologySpInputType;
  ...
  data?: TechnologySpDataBlock;
  buckets?: TechnologySpBucket[];
  items?: TechnologySpItem[];
...
export interface TechnologySpGenerateQuestionRequest {
  skill_id: string;
  used_refs: string[];
  attempt_number?: number;
}

export interface TechnologySpGenerateQuestionResponse {
  question: TechnologySpGeneratedQuestion | null;
}

export interface TechnologySpSubmitAnswerRequest {
  student_id: string;
  question_id: string;
  skill_id: string;
  question_ref: string;
...
export interface TechnologySpSubmitAnswerResponse {
  is_correct: boolean;
  ...
  mastery_update: {
    skill_id: string;
    new_status: TechnologySpMasteryStatus;
    correct_count: number;
    attempt_count: number;
  };
  next_action:
