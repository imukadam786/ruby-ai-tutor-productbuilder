...
//     The topic id IS the atomic skill id (e.g. "EMS.G7.LL.A1").
//
...
export type EmsSpSourceType = "text" | "image" | "map" | "cartoon";

export interface EmsSpProvenance {
  author?: string;
  date?: string;
  origin?: string;
...
export interface EmsSpDataBlock {
...
export interface EmsSpBankQuestion {
   ...
   *  public/ems-sp/<key>.{png|svg|jpg|webp} with a text fallback. */
  image_refs?: string[];
  ...
  data?: EmsSpDataBlock;       // data-interpret

  buckets?: EmsSpBucket[];     // sort-buckets
  items?: EmsSpItem[];         // sort-buckets + sequence
...
export interface EmsSpTopic {
  ...
  questions: EmsSpBankQuestion[];
}

export interface EmsSpBank {
  version: string;
  subject: "ems-sp";
  description?: string;
  /** key = atomic skill id (e.g. "EMS.G7.LL.A1"). */
  topics: Record<string, EmsSpTopic>;
}

// ─── Skill Tree ───────────────────────────────────────────────────────────────

export interface EmsSpErrorSignature {
  type: string;
  description: string;
  example: string;
}

export interface EmsSpAtomicSkill {
  id: string;
  ...
  templates: string[];
  error_signatures: EmsSpErrorSignature[];
  recovery_strategy: string;
  mastery_criteria: {
...
export interface EmsSpTier {
  id: string;                      // "L7.LL"
  title: string;                   // "Term 1 — Life & Living"
  description?: string;
  atomic_skills: EmsSpAtomicSkill[];
}

export interface EmsSpLevel {
  id: number;
  grade: number;
  title: string;
  description: string;
...
export interface EmsSpSkillTree {
  version: string;
  subject: "ems-sp";
  description?: string;
...
export type EmsSpMasteryStatus =
  | "locked"
  | "available"
  | "in_progress"
  | "mastered"
...
export interface EmsSpSkillAttempt {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: EmsSpInputType;
...
export interface EmsSpSkillMastery {
  skill_id: string;
  status: EmsSpMasteryStatus;
  correct_count: number;
  attempt_count: number;
  formats_used: EmsSpInputType[];
  last_attempted: string;
  mastered_at?: string;
  last_reviewed_at?: string;
}

export interface EmsSpStudentProfile {
  id: string;
  name: string;
  grade: number;
  current_level: number;
  current_skill_id: string;
  skill_mastery: Record<string, EmsSpSkillMastery>;
...
export interface EmsSpGeneratedQuestion {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: EmsSpInputType;
...
export interface EmsSpGenerateQuestionRequest {
  skill_id: string;
  used_refs: string[];
  attempt_number?: number;
}

export interface EmsSpGenerateQuestionResponse {
  question: EmsSpGeneratedQuestion | null;
}

export interface EmsSpSubmitAnswerRequest {
  student_id: string;
  question_id: string;
  skill_id: string;
  question_ref: string;
...
export interface EmsSpSubmitAnswerResponse {
  is_correct: boolean;
