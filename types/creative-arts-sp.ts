...
// Senior Phase the IP subject "Natural Sciences & Creative Arts" (Grades 4–6)
// splits — Creative Arts becomes its own subject and this is the Sciences half.
...
export type CreativeArtsSpInputType =
  | "choice"
  | "true-false"
  | "cloze"
  | "scenario"
...
export type CreativeArtsSpSourceType = "text" | "image" | "map" | "cartoon";

export interface CreativeArtsSpProvenance {
  author?: string;
  date?: string;
  origin?: string;
...
export interface CreativeArtsSpSource {
  type: CreativeArtsSpSourceType;
  /** For type "text": the passage. For image/map/cartoon: the image_ref key. */
  content: string;
  provenance?: CreativeArtsSpProvenance;
  caption?: string;
}

// ─── Per-mechanic schema fragments (carried on the bank question) ─────────────

export interface CreativeArtsSpBucket {
  id: string;
  label: string;
}
/** Used by both sort-buckets (with correct_bucket) and sequence (without). */
export interface CreativeArtsSpItem {
  id: string;
  text: string;
  correct_bucket?: string;
}

export interface CreativeArtsSpHighlightTarget {
  phrase: string;
  reason: string;
}
...
export interface CreativeArtsSpDataBlock {
...
export interface CreativeArtsSpBankQuestion {
   ...
   *  public/creative-arts-sp/<key>.{png|svg|jpg|webp} with a text fallback. */
  image_refs?: string[];

  // Optional `source` field — renders above the mechanic.
  source?: CreativeArtsSpSource;

  // Per-mechanic payloads. Exactly one set is populated per question.
  data?: CreativeArtsSpDataBlock;       // data-interpret

  buckets?: CreativeArtsSpBucket[];     // sort-buckets
  items?: CreativeArtsSpItem[];         // sort-buckets + sequence
  ...
  targets?: CreativeArtsSpHighlightTarget[];
...
export type CreativeArtsSpGate = "NONE";

/** One bank entry. Keyed in the bank by the atomic skill id
 *  (e.g. "LO.G7.LL.A1"). */
export interface CreativeArtsSpTopic {
  title: string;
  description?: string;
  grade: number;
  strand: string;
  skill_ids: string[];
  gate: CreativeArtsSpGate;
  ...
  questions: CreativeArtsSpBankQuestion[];
}

export interface CreativeArtsSpBank {
  version: string;
  subject: "creative-arts-sp";
  description?: string;
  /** key = atomic skill id (e.g. "LO.G7.LL.A1"). */
  topics: Record<string, CreativeArtsSpTopic>;
}

// ─── Skill Tree ───────────────────────────────────────────────────────────────

export interface CreativeArtsSpErrorSignature {
  type: string;
  description: string;
  example: string;
}

export interface CreativeArtsSpAtomicSkill {
  id: string;
  ...
  templates: string[];
  error_signatures: CreativeArtsSpErrorSignature[];
  recovery_strategy: string;
  mastery_criteria: {
...
export interface CreativeArtsSpTier {
  id: string;                      // "L7.LL"
  title: string;                   // "Term 1 — Life & Living"
  description?: string;
  atomic_skills: CreativeArtsSpAtomicSkill[];
}

export interface CreativeArtsSpLevel {
  id: number;
  grade: number;
  title: string;
  description: string;
  tiers: CreativeArtsSpTier[];
}

export interface CreativeArtsSpSkillTree {
  version: string;
  subject: "creative-arts-sp";
  description?: string;
  grades_covered?: number[];
  levels: CreativeArtsSpLevel[];
}

// ─── Student Model ────────────────────────────────────────────────────────────

export type CreativeArtsSpMasteryStatus =
  | "locked"
  | "available"
  | "in_progress"
  | "mastered"
...
export interface CreativeArtsSpSkillAttempt {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: CreativeArtsSpInputType;
...
export interface CreativeArtsSpSkillMastery {
  skill_id: string;
  status: CreativeArtsSpMasteryStatus;
  correct_count: number;
  attempt_count: number;
  formats_used: CreativeArtsSpInputType[];
  last_attempted: string;
  mastered_at?: string;
  last_reviewed_at?: string;
}

export interface CreativeArtsSpStudentProfile {
  id: string;
  name: string;
  grade: number;
  current_level: number;
  current_skill_id: string;
  skill_mastery: Record<string, CreativeArtsSpSkillMastery>;
...
export interface CreativeArtsSpGeneratedQuestion {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: CreativeArtsSpInputType;
  ...
  data?: CreativeArtsSpDataBlock;
  buckets?: CreativeArtsSpBucket[];
  items?: CreativeArtsSpItem[];
...
export interface CreativeArtsSpGenerateQuestionRequest {
  skill_id: string;
  used_refs: string[];
  attempt_number?: number;
}

export interface CreativeArtsSpGenerateQuestionResponse {
  question: CreativeArtsSpGeneratedQuestion | null;
}

export interface CreativeArtsSpSubmitAnswerRequest {
  student_id: string;
  question_id: string;
  skill_id: string;
  question_ref: string;
...
export interface CreativeArtsSpSubmitAnswerResponse {
  is_correct: boolean;
  ...
  mastery_update: {
    skill_id: string;
    new_status: CreativeArtsSpMasteryStatus;
    correct_count: number;
    attempt_count: number;
  };
  next_action:
