...
//     The topic id IS the atomic skill id (e.g. "NSSP.G7.LL.A1").
//
...
export type NaturalSciencesSpInputType =
  | "choice"
  | "true-false"
  | "cloze"
  | "scenario"
...
export type NaturalSciencesSpSourceType = "text" | "image" | "map" | "cartoon";

export interface NaturalSciencesSpProvenance {
  author?: string;
  date?: string;
  origin?: string;
...
export interface NaturalSciencesSpSource {
  type: NaturalSciencesSpSourceType;
  /** For type "text": the passage. For image/map/cartoon: the image_ref key. */
  content: string;
  provenance?: NaturalSciencesSpProvenance;
  caption?: string;
}

// ─── Per-mechanic schema fragments (carried on the bank question) ─────────────

export interface NaturalSciencesSpBucket {
  id: string;
  label: string;
}
/** Used by both sort-buckets (with correct_bucket) and sequence (without). */
export interface NaturalSciencesSpItem {
  id: string;
  text: string;
  correct_bucket?: string;
}

export interface NaturalSciencesSpHighlightTarget {
  phrase: string;
  reason: string;
}
...
export interface NaturalSciencesSpDataBlock {
...
export interface NaturalSciencesSpBankQuestion {
   ...
   *  public/natural-sciences-sp/<key>.{png|svg|jpg|webp} with a text fallback. */
  image_refs?: string[];

  // Optional `source` field — renders above the mechanic.
  source?: NaturalSciencesSpSource;

  // Per-mechanic payloads. Exactly one set is populated per question.
  data?: NaturalSciencesSpDataBlock;       // data-interpret

  buckets?: NaturalSciencesSpBucket[];     // sort-buckets
  items?: NaturalSciencesSpItem[];         // sort-buckets + sequence
  ...
  targets?: NaturalSciencesSpHighlightTarget[];
...
export type NaturalSciencesSpGate = "NONE";

/** One bank entry. Keyed in the bank by the atomic skill id
 *  (e.g. "NSSP.G7.LL.A1"). */
export interface NaturalSciencesSpTopic {
  title: string;
  description?: string;
  grade: number;
  strand: string;
  skill_ids: string[];
  gate: NaturalSciencesSpGate;
  ...
  questions: NaturalSciencesSpBankQuestion[];
}

export interface NaturalSciencesSpBank {
  version: string;
  subject: "natural-sciences-sp";
  description?: string;
  /** key = atomic skill id (e.g. "NSSP.G7.LL.A1"). */
  topics: Record<string, NaturalSciencesSpTopic>;
}

// ─── Skill Tree ───────────────────────────────────────────────────────────────

export interface NaturalSciencesSpErrorSignature {
  type: string;
  description: string;
  example: string;
}

export interface NaturalSciencesSpAtomicSkill {
  id: string;
  ...
  templates: string[];
  error_signatures: NaturalSciencesSpErrorSignature[];
  recovery_strategy: string;
  mastery_criteria: {
...
export interface NaturalSciencesSpTier {
  id: string;                      // "L7.LL"
  title: string;                   // "Term 1 — Life & Living"
  description?: string;
  atomic_skills: NaturalSciencesSpAtomicSkill[];
}

export interface NaturalSciencesSpLevel {
  id: number;
  grade: number;
  title: string;
  description: string;
  tiers: NaturalSciencesSpTier[];
}

export interface NaturalSciencesSpSkillTree {
  version: string;
  subject: "natural-sciences-sp";
  description?: string;
  grades_covered?: number[];
  levels: NaturalSciencesSpLevel[];
}

// ─── Student Model ────────────────────────────────────────────────────────────

export type NaturalSciencesSpMasteryStatus =
  | "locked"
  | "available"
  | "in_progress"
  | "mastered"
...
export interface NaturalSciencesSpSkillAttempt {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: NaturalSciencesSpInputType;
...
export interface NaturalSciencesSpSkillMastery {
  skill_id: string;
  status: NaturalSciencesSpMasteryStatus;
  correct_count: number;
  attempt_count: number;
  formats_used: NaturalSciencesSpInputType[];
  last_attempted: string;
  mastered_at?: string;
  last_reviewed_at?: string;
}

export interface NaturalSciencesSpStudentProfile {
  id: string;
  name: string;
  grade: number;
  current_level: number;
  current_skill_id: string;
  skill_mastery: Record<string, NaturalSciencesSpSkillMastery>;
...
export interface NaturalSciencesSpGeneratedQuestion {
  id: string;
  skill_id: string;
  question_ref: string;
  input_type: NaturalSciencesSpInputType;
  ...
  data?: NaturalSciencesSpDataBlock;
  buckets?: NaturalSciencesSpBucket[];
  items?: NaturalSciencesSpItem[];
...
export interface NaturalSciencesSpGenerateQuestionRequest {
  skill_id: string;
  used_refs: string[];
  attempt_number?: number;
}

export interface NaturalSciencesSpGenerateQuestionResponse {
  question: NaturalSciencesSpGeneratedQuestion | null;
}

export interface NaturalSciencesSpSubmitAnswerRequest {
  student_id: string;
  question_id: string;
  skill_id: string;
  question_ref: string;
...
export interface NaturalSciencesSpSubmitAnswerResponse {
  is_correct: boolean;
  ...
  mastery_update: {
    skill_id: string;
    new_status: NaturalSciencesSpMasteryStatus;
    correct_count: number;
    attempt_count: number;
  };
  next_action:
